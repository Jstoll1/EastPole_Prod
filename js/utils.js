...
function cleanPickName(raw) {
  if (!raw) return '';
  return raw.split('|')[0].trim();
}

function parseTiebreaker(raw) {
  if (!raw) return 0;
  var m = raw.match(/(\d+)\s*under/i);
...
function parseEntriesCSV(csvText) {
  ...
  for (var i = 1; i < lines.length; i++) {
    ...
    var email = (cols[1] || '').toLowerCase().trim();
    var teamRaw = (cols[2] || '').trim();
    var name = (cols[3] || '').trim();
    if (!email || !teamRaw || !name) continue;
    var picks = [
      cleanPickName(cols[4]),
      cleanPickName(cols[5]),
      cleanPickName(cols[6]),
      cleanPickName(cols[7]),
      cleanPickName(cols[8]),
      cleanPickName(cols[9])
    ].filter(Boolean);
    if (picks.length < 6) continue;
    var tb = parseTiebreaker(cols[10] || '');
    // Dedupe by team name (trim trailing spaces)
    var teamKey = teamRaw.replace(/\s+$/, '');
    var dupeCount = seen[teamKey + '|' + email] || 0;
    seen[teamKey + '|' + email] = dupeCount + 1;
    var team = dupeCount > 0 ? teamKey + ' ' + (dupeCount + 1) : teamKey;
    entries.push({ team: team, name: name, email: email, picks: picks, tb: tb });
  }
  return entries;
}
...
async function fetchEntriesFromSheets() {
  ...
  try {
    var resp = await fetch(SHEETS_CSV_URL);
    if (!resp.ok) throw new Error('HTTP ' + resp.status);
    var csv = await resp.text();
    var parsed = parseEntriesCSV(csv);
    if (parsed.length >= ENTRIES.length) {
      ENTRIES = parsed;
      ENTRIES.forEach(function(e) { e.picks = e.picks.map(resolvePlayerName); });
      allTeamEmails = Array.from(new Set(ENTRIES.map(function(e) { return e.email; }))).sort();
      OWNERSHIP_DATA = computeOwnership();
      console.log('📊 Loaded', ENTRIES.length, 'entries from Google Sheets');
      return true;
    } else {
      console.warn('⚠️ Sheets returned fewer entries (' + parsed.length + ') than baked-in (' + ENTRIES.length + '), keeping baked-in');
      return false;
    }
  } catch (e) {
    console.warn('⚠️ Sheets fetch failed, using baked-in entries:', e.message);
    return false;
  }
}
