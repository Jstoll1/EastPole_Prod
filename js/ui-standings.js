...
function renderStandings() {
  ...
  if (tourneyDone && ranked[0]) {
    ...
    winnerBoxEl.innerHTML = '<div class="ws-confetti"></div>'
      ...
      + '<div class="ws-floater ws-float-1">⛳</div>'
      + '<div class="ws-floater ws-float-2">🍺</div>'
      + '<div class="ws-floater ws-float-3">🏌️</div>'
      + '<div class="ws-floater ws-float-4">🍻</div>'
      + '<div class="ws-floater ws-float-5">⛳</div>'
      + '<div class="ws-floater ws-float-6">🍺</div>'
    ...
    teamsToShow.forEach(function(activeTeam) {
      ...
      if (myIdx >= 0) {
        ...
        myEntry.top4.forEach(function(g) {
          ...
          t4parts.push('<span class="mh-g">' + flag + lastName + ' <span class="mh-g-tot ' + cls(sc) + '">' + scDisp + '</span></span>');
        });
